...
class GridGameGUI:
  ...
  def __init__(
    ...
    self.cell_size = max(
        20,
        min(
            max_canvas_dim // self.env.width, max_canvas_dim // self.env.height
        ),
    )
    ...
    # Step 1.3 (5): External agent injection (Default SearchAgent with AStar)
    self.search_agent = agent if agent is not None else SearchAgent()
  ...
  def run_loop(self):
    ...
    def step():
      if not self.env.is_done():
        ...
        self.label.config(
            text=(
                f'Score: {self.env.score} | Steps: {self.env.steps} | Action:'
                f' {action}'
            )
        )
...
if __name__ == '__main__':
  root = tk.Tk()
  # Step 1.3 (5): SearchAgent instance එකක් GUI එකට inject කර ධාවනය කිරීම
